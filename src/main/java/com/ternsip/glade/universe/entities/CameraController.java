...
public class CameraController {
    ...
    public CameraController(EntityPlayer entityPlayer) {
        this.target = entityPlayer;
        Universe.INSTANCE.getDisplaySnapReceiver().getDisplayCallbacks().getScrollCallbacks().add(this::recalculateZoom);
        Universe.INSTANCE.getDisplaySnapReceiver().getDisplayCallbacks().getResizeCallbacks().add(this::recalculateProjectionMatrices);
        Universe.INSTANCE.getDisplaySnapReceiver().getDisplayCallbacks().getCursorPosCallbacks().add(this::recalculateRotation);
    ...
    private void recalculateRotation(double xPos, double yPos, double dx, double dy) {
        if (Universe.INSTANCE.getDisplaySnapReceiver().isMouseDown(GLFW_MOUSE_BUTTON_1)) {
